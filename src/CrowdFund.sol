...
contract CrowdFund is Ownable {
    ...
    modifier onlyContributors() {
        require(contributorsToAmountContributed[msg.sender] > 0, "User is not a contributor: Contribute to enter");
        _;
    }
    ...
    function createCrownFundCampaign(address projectOwner, uint256 durationOfRaise, uint256 targetAmount)
        public
        returns (uint256 crowdFundId)
    {
        ...
        require(projectOwner != address(0), "zero address cannot host campaign");
    ...
    function updateProtocolOwner(address newProtocolOwner) external onlyOwner {
        require(newProtocolOwner != address(0), "new protocol owner cannot be zero address");
        newProtocolOwner = protocol;
    } // needed for testing - should be removed.

    function calculateProtocolFees(uint256 _contribution) internal pure returns (uint256) {
        if (_contribution == 0) {
    ...
    function hostProposal(string memory _description, string memory _achievement, address project) public {
        require(
            block.timestamp >= lastProposalTimeByProject[msg.sender] + INTERVAL_BETWEEN_PROPOSALS,
            "cannot host another proposal yet."
        );

        require(bytes(_description).length > 0, "description cannot be empty string");
        require(bytes(_achievement).length > 0, "achievement cannot be empty string");

        require(project != address(0), "project owner cannot be zero address");
        require(msg.sender == project, "only project owner can host proposal");

        MileStoneProposalDetails memory milestoneProposal = MileStoneProposalDetails({
    ...
    function withdrawProtocolFees() external onlyProtocol {
        ...
        // Interactions
        (bool success,) = protocolFeeAddress.call{value: totalProtocolFees}(""); // this should be sent to protocolFees address - will implement
        if (!success) {
    ...
    function withdrawFundsRaised(uint256 amountToWithdraw) external onlyProtocol {
    ...
    function getProposals(address __projectOwner) public view returns (MileStoneProposalDetails[] memory) {
    ...
    function getMilestoneProposalDetails(address _project) public view returns (MileStoneProposalDetails[] memory) {
        return proposals;
    }
}
